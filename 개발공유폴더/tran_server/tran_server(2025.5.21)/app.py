from flask import Flask, request, send_file, render_template, jsonify, make_response, redirect, Response
import requests
import tempfile
import os
...
DEPLOY_SERVER_URL = "http://localhost:7070"
EXTERNAL_SERVER_URL = 'http://localhost:8080/upload'
DEPLOY_URL = 'http://localhost:7070/download'
USERNAME = None
KEY = SECRET_KEY
...
def check_update():
    ...
    try:
        manifest_url = f"{DEPLOY_SERVER_URL}/manifest"
...
def download_file(filename):
    try:
        # 배포 서버 API를 통해 파일 요청
        file_url = f"{DEPLOY_SERVER_URL}/file/{filename}"
...
def download_page():
    return render_template('download_installer.html')

@app.route('/download_installer')
def download_installer():
    # 배포 서버로부터 설치 파일 요청
    r = requests.get(DEPLOY_URL, stream=True)
    return Response(
        r.iter_content(chunk_size=8192),
        content_type=r.headers['Content-Type'],
        headers={
            "Content-Disposition": r.headers.get("Content-Disposition", "attachment; filename=installer.exe")
        }
    )
